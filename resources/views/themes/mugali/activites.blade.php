...
@section('content')
    ...
    {{-- Gallery Scroll --}}
    @php
        $dedicatedGallery = collect($activitesGallerySetting->gallery ?? []);
        $galleryImages = $dedicatedGallery->isNotEmpty()
            ? $dedicatedGallery
            : ($installations ?? collect())->filter(fn($i) => !empty($i->image_path))->pluck('image_path')->values();
    @endphp
    <section class="galleryscroll section-padding bg-lightbrown">
        <div class="container-fluid p-0">
            <div class="row justify-content-center">
                <div class="col-md-12 text-center mb-20">
                    <div class="section-subtitle">{{ method_exists($activitesGallerySetting, 't') ? $activitesGallerySetting->t('small_title') : ($activitesGallerySetting->small_title) }}</div>
                    <div class="section-title">{{ method_exists($activitesGallerySetting, 't') ? $activitesGallerySetting->t('title') : ($activitesGallerySetting->title) }}</div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="owl-carousel owl-theme">
                        @if($galleryImages->isNotEmpty())
                            ...
                            <div class="item">
                                <a href="{{ asset('storage/' . $gItem) }}" title="" class="gallery-masonry-item-img-link img-zoom">
                                    <div class="img"><img src="{{ asset('storage/' . $gItem) }}" class="img-fluid mx-auto d-block" alt=""></div>
                                </a>
                            </div>
                            @endforeach
                        @else
